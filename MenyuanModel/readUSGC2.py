...
lonStart = np.max( LonCoord[0, 0] )
lonEnd = np.min( LonCoord[0, -1])

latStart = np.max( LatCoord [0, 0])
latEnd = np.min( LatCoord[0, -1])

print( "lonStart = %f, lonEnd = %f, latStart = %f, latEnd = %f" % (  lonStart, lonEnd, latStart, latEnd )  )

lonStart = np.max( LonCoord[-1, 0] )
lonEnd = np.min( LonCoord[-1, -1])

latStart = np.max( LatCoord [-1, 0])
latEnd = np.min( LatCoord[-1, -1])
