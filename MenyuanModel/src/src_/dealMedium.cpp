...
void writeVs3D( PARAMS params, GRID grid, MPI_COORD thisMPICoord, float * Vs)
{
		
	
	char fileName[256];

	sprintf( fileName, "%s/Vs_mpi_%d_%d_%d.bin", params.OUT, thisMPICoord.X, thisMPICoord.Y, thisMPICoord.Z );
...
#define StructureTerrain
	...
	writeVs3D( params, grid, thisMPICoord, structure.Vs  );
	free( Vs );
