...
void modelChecking( PARAMS params  )
{
	...
	if ( params.useTerrain && params.gauss_hill )
	{
		printf( 
		"!!!!!!!!!!!!!!!!!!!!!!!WORNING!!!!!!!!!!!!!!!!!!!!!!!"
		"You set \"useTerrain\" and \"gauss_hill\" can not be 1 at the same time. You should configue your \"./paramsDir/paramsCGFDM3D-LSRK-CJM.json\" file. Or, we will use \"gauss_hill\" model to run the simulation\n"
		"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		params.gauss_hill = 1;
		params.useTerrain = 0;
...
void run( MPI_Comm comm_cart, MPI_COORD thisMPICoord, MPI_NEIGHBOR mpiNeighbor, GRID grid, PARAMS params )
{
	...
	solveContravariantJac( comm_cart, mpiNeighbor, grid, sr, con, coord, Jac, medium, _rDZ_DX, _rDZ_DY );
#else 
	solveContravariantJac( comm_cart, mpiNeighbor, grid, sr, con, coord, Jac );
#endif
	freeCoord( coord );
#ifdef GPU_CUDA
