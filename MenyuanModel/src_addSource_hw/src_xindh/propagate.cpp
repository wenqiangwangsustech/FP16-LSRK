...
SLICE slice, SLICE_DATA sliceData, SLICE_DATA sliceDataCpu )
{
	...
	if ( IsFreeSurface )
	{
		...
		allocatePGA( grid, &pga );
		allocateGround( grid, &ground );
		if ( sliceFreeSurf )
			allocSliceData( grid, freeSurfSlice, &freeSurfData );
#ifdef GPU_CUDA
		allocatePGV_cpu( grid, &cpuPgv );
		allocatePGA_cpu( grid, &cpuPga );
		if ( sliceFreeSurf )
	...
	*/
	...
	for ( it = 0; it < NT; it ++ )
	{
		...
		if ( IsFreeSurface ) 	
		{
			comparePGV( grid, thisMPICoord, W, pgv );
			calculatePga( grid, thisMPICoord, W, pga, ground, DT );
		}
	...
	if ( IsFreeSurface )
	{
		...
		outputPGA( params, grid, thisMPICoord, pga, cpuPga );
		freePGV( pgv );
		freePGA( pga );
		freeGround( ground );
		if ( sliceFreeSurf ) 
