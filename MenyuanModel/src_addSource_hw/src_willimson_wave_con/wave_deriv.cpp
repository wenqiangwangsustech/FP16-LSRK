...
void allocWave( GRID grid, WAVE * h_W, WAVE * W, WAVE * t_W, WAVE * m_W )
{
	...
	FLOAT * pWave = NULL;
	long long size = sizeof( FLOAT ) * num * WAVESIZE * 2; 
...
{																	
	...
	CALCULATE3D( i, j, k, HALO, _nx, HALO, _ny, HALO, _nz )  
		...
		h_WVx  	= A * (float)h_W.Vx [index] +  ( Vx1  + Vx2  + Vx3  ) * DT;							
		h_WVy  	= A * (float)h_W.Vy [index] +  ( Vy1  + Vy2  + Vy3  ) * DT;							
		h_WVz  	= A * (float)h_W.Vz [index] +  ( Vz1  + Vz2  + Vz3  ) * DT;							
		h_WTxx 	= A * (float)h_W.Txx[index] +  ( Txx1 + Txx2 + Txx3 ) * DT;						
		h_WTyy 	= A * (float)h_W.Tyy[index] +  ( Tyy1 + Tyy2 + Tyy3 ) * DT;						
		h_WTzz 	= A * (float)h_W.Tzz[index] +  ( Tzz1 + Tzz2 + Tzz3 ) * DT;						
		h_WTxy 	= A * (float)h_W.Txy[index] +  ( Txy1 + Txy2 + Txy3 ) * DT;						
		h_WTxz 	= A * (float)h_W.Txz[index] +  ( Txz1 + Txz2 + Txz3 ) * DT;						
		h_WTyz 	= A * (float)h_W.Tyz[index] +  ( Tyz1 + Tyz2 + Tyz3 ) * DT;						
...
void waveDeriv( GRID grid, WAVE h_W, WAVE W, CONTRAVARIANT_FLOAT con, MEDIUM_FLOAT medium, 
	...
	PML_BETA pml_beta,
#endif
	int irk, int FB1, int FB2, int FB3, float A, float B, float DT, int IsFreeSurface )	
{
	int _nx_ = grid._nx_;
	int _ny_ = grid._ny_;
	int _nz_ = grid._nz_;
