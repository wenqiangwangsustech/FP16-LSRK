...
void storage_station( int stationNum, STATION station, WAVE W, int _nx_, int _ny_, int _nz_, int NT, int it )
{
	...
	CALCULATE1D( i, 0, stationNum ) 
		...
		station.wave.Vx [pos] = (float)W.Vx [index] * C1;
		station.wave.Vy [pos] = (float)W.Vy [index] * C1;
		station.wave.Vz [pos] = (float)W.Vz [index] * C1;
		station.wave.Txx[pos] = (float)W.Txx[index] * C2;
		station.wave.Tyy[pos] = (float)W.Tyy[index] * C2;
		station.wave.Tzz[pos] = (float)W.Tzz[index] * C2;
		station.wave.Txy[pos] = (float)W.Txy[index] * C2;
		station.wave.Txz[pos] = (float)W.Txz[index] * C2;
		station.wave.Tyz[pos] = (float)W.Tyz[index] * C2;
