...
void pml_free_surface_x(		 									
	...
	float *  pml_d_x, int nPML, int _nx_, int _ny_, int _nz_, int FLAG, float rDH, int FB1, float A, float B, float DT )
{																	
#ifdef GPU_CUDA
	...
	CALCULATE2D( i0, j0, 0, nPML, 0, ny )		
		...
		h_Aux_x.Txx[pos] += DT * Txx3;																									
		h_Aux_x.Tyy[pos] += DT * Tyy3;																									
		h_Aux_x.Tzz[pos] += DT * Tzz3;																									
		h_Aux_x.Txy[pos] += DT * Txy3;																									
		h_Aux_x.Txz[pos] += DT * Txz3;																									
		h_Aux_x.Tyz[pos] += DT * Tyz3;																									
...
void pml_free_surface_y( 											
	...
	FLOAT * ZT_X, FLOAT * ZT_Y, FLOAT * ZT_Z, MEDIUM_FLOAT medium, 		
	Mat3x3 _rDZ_DX, Mat3x3 _rDZ_DY,									
	float *  pml_d_y, int nPML, int _nx_, int _ny_, int _nz_, int FLAG, float rDH, int FB2, float A, float B, float DT )
{																	
#ifdef GPU_CUDA
	int i0 = threadIdx.x + blockIdx.x * blockDim.x;
	int j0 = threadIdx.y + blockIdx.y * blockDim.y;
	...
	CALCULATE2D( i0, j0, 0, nx, 0, nPML )	
		...
		h_Aux_y.Txx[pos] += DT * Txx3;																									
		h_Aux_y.Tyy[pos] += DT * Tyy3;																									
		h_Aux_y.Tzz[pos] += DT * Tzz3;																									
		h_Aux_y.Txy[pos] += DT * Txy3;																									
		h_Aux_y.Txz[pos] += DT * Txz3;																									
		h_Aux_y.Tyz[pos] += DT * Tyz3;																									
...
void pmlFreeSurfaceDeriv( GRID grid, WAVE h_W, WAVE W, 
						  ...
						  Mat3x3 _rDZ_DX, Mat3x3 _rDZ_DY,	
						  PML_D pml_d, MPI_BORDER border, int irk, int FB1, int FB2, float A, float B, float DT )
{
	int _nx_ = grid._nx_;
	int _ny_ = grid._ny_;
	int _nz_ = grid._nz_;
	...
	if ( border.isx1 ) pml_free_surface_x<<< blockX, thread >>>	
					  ( h_W, W, 
						...
						pml_d_x, nPML, 
						_nx_, _ny_, _nz_, 0, rDH, FB1 , A, B, DT );
	...
	if ( border.isy1 ) pml_free_surface_y<<< blockY, thread >>>
					  ( h_W, W, 
						...
						medium, _rDZ_DX, _rDZ_DY,				
						pml_d_y, nPML, 
						_nx_, _ny_, _nz_, 0, rDH, FB2 , A, B, DT );
	...
	if ( border.isx2 ) pml_free_surface_x<<< blockX, thread >>>
	                  ( h_W, W, 
						...
						medium, _rDZ_DX, _rDZ_DY,	
						pml_d_x, nPML, 
						_nx_, _ny_, _nz_, 1, rDH, FB1 , A, B, DT );
	...
	if ( border.isy2 ) pml_free_surface_y<<< blockY, thread >>>	
					  ( h_W, W, 
						...
						medium, _rDZ_DX, _rDZ_DY,				
						pml_d_y, nPML, 
						_nx_, _ny_, _nz_, 1, rDH, FB2 , A, B, DT );
#else
