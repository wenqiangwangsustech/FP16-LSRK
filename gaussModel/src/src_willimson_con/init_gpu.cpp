...
void init_gpu( int PX, int PY, int PZ  )
{
	...
	strcpy( jsonFile, "./paramsDir/paramsCGFDM3D-LSRK-CJM.json" );
	FILE * fp;
	fp = fopen( jsonFile, "r" );
	...
	for ( i = 0; i < nodeCnt; i ++  )
	{
		...
		//printf( "nodeObj->string:%s, thisMPINodeName:%s\n", nodeObj->string, thisMPINodeName );
