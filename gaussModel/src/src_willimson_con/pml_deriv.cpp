...
void allocAuxPML( int nPML, int N1, int N2, AUX * h_Aux, AUX * Aux, AUX * t_Aux, AUX * m_Aux )
{
	...
	float * pAux = NULL;
	long long size = sizeof( float ) * num * 9 * 2;
...
void pml_deriv_x( 									
	...
	int nPML, int _nx_, int _ny_, int _nz_, int FLAG, float rDH, int FB1, float A, float B, float DT )
{																
#ifdef GPU_CUDA
	...
	CALCULATE3D( i0, j0, k0, 0, nPML, 0, ny, 0, nz )
		...
		h_Aux_x.Vx [pos] = A * h_Aux_x.Vx [pos] + DT * ( Vx1  - alpha_d_x * Aux_x.Vx [pos] );
		h_Aux_x.Vy [pos] = A * h_Aux_x.Vy [pos] + DT * ( Vy1  - alpha_d_x * Aux_x.Vy [pos] );
		h_Aux_x.Vz [pos] = A * h_Aux_x.Vz [pos] + DT * ( Vz1  - alpha_d_x * Aux_x.Vz [pos] );
		h_Aux_x.Txx[pos] = A * h_Aux_x.Txx[pos] + DT * ( Txx1 - alpha_d_x * Aux_x.Txx[pos] );
		h_Aux_x.Tyy[pos] = A * h_Aux_x.Tyy[pos] + DT * ( Tyy1 - alpha_d_x * Aux_x.Tyy[pos] );
		h_Aux_x.Tzz[pos] = A * h_Aux_x.Tzz[pos] + DT * ( Tzz1 - alpha_d_x * Aux_x.Tzz[pos] );
		h_Aux_x.Txy[pos] = A * h_Aux_x.Txy[pos] + DT * ( Txy1 - alpha_d_x * Aux_x.Txy[pos] );
		h_Aux_x.Txz[pos] = A * h_Aux_x.Txz[pos] + DT * ( Txz1 - alpha_d_x * Aux_x.Txz[pos] );
		h_Aux_x.Tyz[pos] = A * h_Aux_x.Tyz[pos] + DT * ( Tyz1 - alpha_d_x * Aux_x.Tyz[pos] );
															
		h_W.Vx [index] += DT * (- beta_x * Aux_x.Vx [pos] );		
		h_W.Vy [index] += DT * (- beta_x * Aux_x.Vy [pos] );		
		h_W.Vz [index] += DT * (- beta_x * Aux_x.Vz [pos] );		
		h_W.Txx[index] += DT * (- beta_x * Aux_x.Txx[pos] );		
		h_W.Tyy[index] += DT * (- beta_x * Aux_x.Tyy[pos] );		
		h_W.Tzz[index] += DT * (- beta_x * Aux_x.Tzz[pos] );		
		h_W.Txy[index] += DT * (- beta_x * Aux_x.Txy[pos] );		
		h_W.Txz[index] += DT * (- beta_x * Aux_x.Txz[pos] );		
		h_W.Tyz[index] += DT * (- beta_x * Aux_x.Tyz[pos] );
...
void pml_deriv_y( 
	...
	int nPML, int _nx_, int _ny_, int _nz_, int FLAG, float rDH, int FB2, float A, float B, float DT  )	
{																	
	...
	CALCULATE3D( i0, j0, k0, 0, nx, 0, nPML, 0, nz )
		...
		h_Aux_y.Vx [pos] = A * h_Aux_y.Vx [pos] + DT * ( Vx2  - alpha_d_y * Aux_y.Vx [pos] );		
		h_Aux_y.Vy [pos] = A * h_Aux_y.Vy [pos] + DT * ( Vy2  - alpha_d_y * Aux_y.Vy [pos] );		
		h_Aux_y.Vz [pos] = A * h_Aux_y.Vz [pos] + DT * ( Vz2  - alpha_d_y * Aux_y.Vz [pos] );		
		h_Aux_y.Txx[pos] = A * h_Aux_y.Txx[pos] + DT * ( Txx2 - alpha_d_y * Aux_y.Txx[pos] );		
		h_Aux_y.Tyy[pos] = A * h_Aux_y.Tyy[pos] + DT * ( Tyy2 - alpha_d_y * Aux_y.Tyy[pos] );		
		h_Aux_y.Tzz[pos] = A * h_Aux_y.Tzz[pos] + DT * ( Tzz2 - alpha_d_y * Aux_y.Tzz[pos] );		
		h_Aux_y.Txy[pos] = A * h_Aux_y.Txy[pos] + DT * ( Txy2 - alpha_d_y * Aux_y.Txy[pos] );		
		h_Aux_y.Txz[pos] = A * h_Aux_y.Txz[pos] + DT * ( Txz2 - alpha_d_y * Aux_y.Txz[pos] );		
		h_Aux_y.Tyz[pos] = A * h_Aux_y.Tyz[pos] + DT * ( Tyz2 - alpha_d_y * Aux_y.Tyz[pos] );		
																	
		h_W.Vx [index] += DT * ( - beta_y * Aux_y.Vx [pos] );				
		h_W.Vy [index] += DT * ( - beta_y * Aux_y.Vy [pos] );				
		h_W.Vz [index] += DT * ( - beta_y * Aux_y.Vz [pos] );				
		h_W.Txx[index] += DT * ( - beta_y * Aux_y.Txx[pos] );				
		h_W.Tyy[index] += DT * ( - beta_y * Aux_y.Tyy[pos] );				
		h_W.Tzz[index] += DT * ( - beta_y * Aux_y.Tzz[pos] );				
		h_W.Txy[index] += DT * ( - beta_y * Aux_y.Txy[pos] );				
		h_W.Txz[index] += DT * ( - beta_y * Aux_y.Txz[pos] );				
		h_W.Tyz[index] += DT * ( - beta_y * Aux_y.Tyz[pos] );			
...
void pml_deriv_z( 
	...
	int nPML, int _nx_, int _ny_, int _nz_, int FLAG, float rDH, int FB3, float A, float B, float DT )				
{																
	...
	CALCULATE3D( i0, j0, k0, 0, nx, 0, ny, 0, nPML )
		...
		h_Aux_z.Vx [pos] = A * h_Aux_z.Vx [pos] + DT * ( Vx3  - alpha_d_z * Aux_z.Vx [pos] );	
		h_Aux_z.Vy [pos] = A * h_Aux_z.Vy [pos] + DT * ( Vy3  - alpha_d_z * Aux_z.Vy [pos] );	
		h_Aux_z.Vz [pos] = A * h_Aux_z.Vz [pos] + DT * ( Vz3  - alpha_d_z * Aux_z.Vz [pos] );	
		h_Aux_z.Txx[pos] = A * h_Aux_z.Txx[pos] + DT * ( Txx3 - alpha_d_z * Aux_z.Txx[pos] );	
		h_Aux_z.Tyy[pos] = A * h_Aux_z.Tyy[pos] + DT * ( Tyy3 - alpha_d_z * Aux_z.Tyy[pos] );	
		h_Aux_z.Tzz[pos] = A * h_Aux_z.Tzz[pos] + DT * ( Tzz3 - alpha_d_z * Aux_z.Tzz[pos] );	
		h_Aux_z.Txy[pos] = A * h_Aux_z.Txy[pos] + DT * ( Txy3 - alpha_d_z * Aux_z.Txy[pos] );	
		h_Aux_z.Txz[pos] = A * h_Aux_z.Txz[pos] + DT * ( Txz3 - alpha_d_z * Aux_z.Txz[pos] );	
		h_Aux_z.Tyz[pos] = A * h_Aux_z.Tyz[pos] + DT * ( Tyz3 - alpha_d_z * Aux_z.Tyz[pos] );	
															 	
		h_W.Vx [index] += DT * ( - beta_z * Aux_z.Vx [pos] );			
		h_W.Vy [index] += DT * ( - beta_z * Aux_z.Vy [pos] );			
		h_W.Vz [index] += DT * ( - beta_z * Aux_z.Vz [pos] );			
		h_W.Txx[index] += DT * ( - beta_z * Aux_z.Txx[pos] );			
		h_W.Tyy[index] += DT * ( - beta_z * Aux_z.Tyy[pos] );			
		h_W.Tzz[index] += DT * ( - beta_z * Aux_z.Tzz[pos] );			
		h_W.Txy[index] += DT * ( - beta_z * Aux_z.Txy[pos] );			
		h_W.Txz[index] += DT * ( - beta_z * Aux_z.Txz[pos] );			
		h_W.Tyz[index] += DT * ( - beta_z * Aux_z.Tyz[pos] );			

	END_CALCULATE3D( )											
}


void pmlDeriv( GRID grid, WAVE h_W, WAVE W, CONTRAVARIANT_FLOAT con, MEDIUM_FLOAT medium, AUX4 Aux4_1, AUX4 Aux4_2, PML_ALPHA pml_alpha, PML_BETA pml_beta, PML_D pml_d, MPI_BORDER border, int irk, int FB1, int FB2, int FB3, float A, float B, float DT )

{
	...
	if ( border.isx1 )  pml_deriv_x<<< blockX, thread >>>( h_W, W, Aux4_1.h_Aux_x, Aux4_1.Aux_x, XI_X, XI_Y, XI_Z, medium, pml_alpha_x, pml_beta_x, pml_d_x, nPML, _nx_, _ny_, _nz_, 0, rDH, FB1 , A, B, DT );
	if ( border.isy1 )  pml_deriv_y<<< blockY, thread >>>( h_W, W, Aux4_1.h_Aux_y, Aux4_1.Aux_y, ET_X, ET_Y, ET_Z, medium, pml_alpha_y, pml_beta_y, pml_d_y, nPML, _nx_, _ny_, _nz_, 0, rDH, FB2 , A, B, DT );
	if ( border.isz1 )  pml_deriv_z<<< blockZ, thread >>>( h_W, W, Aux4_1.h_Aux_z, Aux4_1.Aux_z, ZT_X, ZT_Y, ZT_Z, medium, pml_alpha_z, pml_beta_z, pml_d_z, nPML, _nx_, _ny_, _nz_, 0, rDH, FB3 , A, B, DT );


	if ( border.isx2 )  pml_deriv_x<<< blockX, thread >>>( h_W, W, Aux4_2.h_Aux_x, Aux4_2.Aux_x, XI_X, XI_Y, XI_Z, medium, pml_alpha_x, pml_beta_x, pml_d_x, nPML, _nx_, _ny_, _nz_, 1, rDH, FB1 , A, B, DT );
	if ( border.isy2 )  pml_deriv_y<<< blockY, thread >>>( h_W, W, Aux4_2.h_Aux_y, Aux4_2.Aux_y, ET_X, ET_Y, ET_Z, medium, pml_alpha_y, pml_beta_y, pml_d_y, nPML, _nx_, _ny_, _nz_, 1, rDH, FB2 , A, B, DT );
#ifndef FREE_SURFACE
	if ( border.isz2 )  pml_deriv_z<<< blockZ, thread >>>( h_W, W, Aux4_2.h_Aux_z, Aux4_2.Aux_z, ZT_X, ZT_Y, ZT_Z, medium, pml_alpha_z, pml_beta_z, pml_d_z, nPML, _nx_, _ny_, _nz_, 1, rDH, FB3 , A, B, DT );
#endif
	...
	if ( border.isx1 )  pml_deriv_x( h_W, W, Aux4_1.h_Aux_x, Aux4_1.Aux_x, XI_X, XI_Y, XI_Z, medium, pml_alpha_x, pml_beta_x, pml_d_x, nPML, _nx_, _ny_, _nz_, 0, rDH, FB1 , A, B, DT );
	if ( border.isy1 )  pml_deriv_y( h_W, W, Aux4_1.h_Aux_y, Aux4_1.Aux_y, ET_X, ET_Y, ET_Z, medium, pml_alpha_y, pml_beta_y, pml_d_y, nPML, _nx_, _ny_, _nz_, 0, rDH, FB2 , A, B, DT );
	if ( border.isz1 )  pml_deriv_z( h_W, W, Aux4_1.h_Aux_z, Aux4_1.Aux_z, ZT_X, ZT_Y, ZT_Z, medium, pml_alpha_z, pml_beta_z, pml_d_z, nPML, _nx_, _ny_, _nz_, 0, rDH, FB3 , A, B, DT );


	if ( border.isx2 )  pml_deriv_x( h_W, W, Aux4_2.h_Aux_x, Aux4_2.Aux_x, XI_X, XI_Y, XI_Z, medium, pml_alpha_x, pml_beta_x, pml_d_x, nPML, _nx_, _ny_, _nz_, 1, rDH, FB1 , A, B, DT );
	if ( border.isy2 )  pml_deriv_y( h_W, W, Aux4_2.h_Aux_y, Aux4_2.Aux_y, ET_X, ET_Y, ET_Z, medium, pml_alpha_y, pml_beta_y, pml_d_y, nPML, _nx_, _ny_, _nz_, 1, rDH, FB2 , A, B, DT );
#ifndef FREE_SURFACE
	if ( border.isz2 )  pml_deriv_z( h_W, W, Aux4_2.h_Aux_z, Aux4_2.Aux_z, ZT_X, ZT_Y, ZT_Z, medium, pml_alpha_z, pml_beta_z, pml_d_z, nPML, _nx_, _ny_, _nz_, 1, rDH, FB3 , A, B, DT );
#endif
