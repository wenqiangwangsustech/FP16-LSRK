	...
	for key in station.keys( ):
		...
		for iKey in Keys:
			if key == iKey:
				...
				i = stationKeyNum[key]
				if i == 0:
					plt.plot( t, U[version, i] / vmax + i * disWid, color = colors[version], ls = lines[version], label = NameMode[version], linewidth = lineWidth )
					if version == 0:
						plt.text( TMAX * 0.85, i * disWid + 0.05 * disWid, "%.2fcm/s" % np.max( np.abs( U[version, i] * 100 ) ) )
					if version != 0:
						plt.plot( t, (U[version, i] - U[0, i] ) * zoom / vmax + i * disWid, color = colors[version], ls = linesDiff[version], alpha = 0.5, label = NameMode[version] + ':Diff $ \\times $ %d' % zoom , linewidth = lineWidth)

					plt.legend( loc = 1 )
				else:
					plt.plot( t, U[version, i] / vmax + i * disWid, color = colors[version], ls = lines[version], linewidth = lineWidth )
					if version == 0:
						plt.text( TMAX * 0.85, i * disWid + 0.05 * disWid, "%.2fcm/s" % np.max( np.abs( U[version, i] * 100 ) ) )
					if version != 0:
						plt.plot( t, ( U[version, i] - U[0, i] ) * zoom / vmax + i * disWid, color = colors[version], ls = linesDiff[version], alpha = 0.5, linewidth = lineWidth )
...
plt.gca( ).set_ylim( [-1, StationTicks[-1] * disWid - 0.5 * disWid ] )
...
if var == 'Vx':
	plt.title( "Gauss Topography: $v_x$" )
if var == 'Vy':
	plt.title( "Gauss Topography: $v_y$" )
if var == 'Vz':
	plt.title( "Gauss Topography: $v_z$" )
plt.xlabel( 't(s)'  )

#plt.show( )
plt.savefig( "PDF/GaussTopography%s.pdf" % ( var ), bbox_inches = "tight"  )
