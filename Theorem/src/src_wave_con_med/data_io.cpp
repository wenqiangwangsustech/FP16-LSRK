...
__GLOBAL__
void pack_iodata_x( void * datain, float * dataout, int nx, int ny, int nz, int I, int CVS, int FP_TYPE )
{
	int _nx_ = nx + 2 * HALO;
	int _ny_ = ny + 2 * HALO;
	int _nz_ = nz + 2 * HALO;
...
__GLOBAL__
void pack_iodata_y( void * datain, float * dataout, int nx, int ny, int nz, int J, int CVS, int FP_TYPE )
{
	int _nx_ = nx + 2 * HALO;
	int _ny_ = ny + 2 * HALO;
	int _nz_ = nz + 2 * HALO;
...
__GLOBAL__
void pack_iodata_z( void * datain, float * dataout, int nx, int ny, int nz, int K, int CVS, int FP_TYPE )
{
	int _nx_ = nx + 2 * HALO;
	int _ny_ = ny + 2 * HALO;
	int _nz_ = nz + 2 * HALO;
...
void data2D_output_bin( GRID grid, SLICE slice, 
						...
						void * datain, SLICE_DATA sliceData, SLICE_DATA sliceDataCpu,	
						const char * name, int CVS, int FP_TYPE )
{
	...
	if ( slice.X >= HALO && slice.X < _nx )
	{
		...
		pack_iodata_x
		<<< blocks, threads >>>
		( datain, sliceData.x, nx, ny, nz, slice.X, CVS, FP_TYPE );
		long long size = sizeof( float ) * ny * nz;
		CHECK( cudaMemcpy( sliceDataCpu.x, sliceData.x, size, cudaMemcpyDeviceToHost ) );

#else
		pack_iodata_x( datain, sliceData.x, nx, ny, nz, slice.X, CVS, FP_TYPE );
	...
	if ( slice.Y >= HALO && slice.Y < _ny )
	{
		...
		pack_iodata_y
		<<< blocks, threads >>>
		( datain, sliceData.y, nx, ny, nz, slice.Y, CVS, FP_TYPE );
		long long size = sizeof( float ) * nx * nz;
		CHECK( cudaMemcpy( sliceDataCpu.y, sliceData.y, size, cudaMemcpyDeviceToHost ) );
#else
		pack_iodata_y( datain, sliceData.y, nx, ny, nz, slice.Y, CVS, FP_TYPE );
	...
	if ( slice.Z >= HALO && slice.Z < _nz )
	{
		...
		pack_iodata_z
		<<< blocks, threads >>>
		( datain, sliceData.z, nx, ny, nz, slice.Z, CVS, FP_TYPE );
		long long size = sizeof( float ) * nx * ny;
		CHECK( cudaMemcpy( sliceDataCpu.z, sliceData.z, size, cudaMemcpyDeviceToHost ) );
#else
		pack_iodata_z( datain, sliceData.z, nx, ny, nz, slice.Z, CVS, FP_TYPE );
		...
		case 'V':
			{
				...
				data2D_output_bin( grid, slice, thisMPICoord, W.Vx, sliceData, sliceDataCpu, VxFileName, 1, FP_TYPE );
				data2D_output_bin( grid, slice, thisMPICoord, W.Vy, sliceData, sliceDataCpu, VyFileName, 1, FP_TYPE );
				data2D_output_bin( grid, slice, thisMPICoord, W.Vz, sliceData, sliceDataCpu, VzFileName, 1, FP_TYPE );
		...
		case 'T':
			{
				...
				data2D_output_bin( grid, slice, thisMPICoord, W.Txx, sliceData, sliceDataCpu, TxxFileName, 2 , FP_TYPE );
				data2D_output_bin( grid, slice, thisMPICoord, W.Tyy, sliceData, sliceDataCpu, TyyFileName, 2 , FP_TYPE );
				data2D_output_bin( grid, slice, thisMPICoord, W.Tzz, sliceData, sliceDataCpu, TzzFileName, 2 , FP_TYPE );
                                                                                                         
				data2D_output_bin( grid, slice, thisMPICoord, W.Txy, sliceData, sliceDataCpu, TxyFileName, 2 , FP_TYPE );
				data2D_output_bin( grid, slice, thisMPICoord, W.Txz, sliceData, sliceDataCpu, TxzFileName, 2 , FP_TYPE );
				data2D_output_bin( grid, slice, thisMPICoord, W.Tyz, sliceData, sliceDataCpu, TyzFileName, 2 , FP_TYPE );
...
void data2D_Model_out( MPI_COORD thisMPICoord, PARAMS params, GRID grid, COORD coord, MEDIUM medium, SLICE slice, SLICE_DATA sliceData, SLICE_DATA sliceDataCpu )
{
...

	{
		...
		data2D_output_bin( grid, slice, thisMPICoord, coord.x, sliceData, sliceDataCpu, XName, 0 , FP_TYPE );
		data2D_output_bin( grid, slice, thisMPICoord, coord.y, sliceData, sliceDataCpu, YName, 0 , FP_TYPE );
		data2D_output_bin( grid, slice, thisMPICoord, coord.z, sliceData, sliceDataCpu, ZName, 0 , FP_TYPE );
	...
	}
	{
		...
		data2D_output_bin( grid, slice, thisMPICoord, medium.mu, sliceData, sliceDataCpu, XName, 0 , FP_TYPE );
		data2D_output_bin( grid, slice, thisMPICoord, medium.lambda, sliceData, sliceDataCpu, YName, 0 , FP_TYPE );
		data2D_output_bin( grid, slice, thisMPICoord, medium.buoyancy, sliceData, sliceDataCpu, ZName, 0 , FP_TYPE );
	}
