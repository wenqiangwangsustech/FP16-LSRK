...
__GLOBAL__
void calculate_MomentRate( long long npts, long long nt, MEDIUM_FLOAT medium, FLOAT * Jac, MOMENT_RATE momentRate, long long * srcIndex, float DH )
{
	...
	CALCULATE2D( i, j, 0, npts, 0, nt )
		...
		V = 1.0 / V;
		V = V * Cv;
		momentRate.Mxx[pos] *= ( float )medium.mu[idx] * V;
		momentRate.Myy[pos] *= ( float )medium.mu[idx] * V;
		momentRate.Mzz[pos] *= ( float )medium.mu[idx] * V;
		momentRate.Mxy[pos] *= ( float )medium.mu[idx] * V;
		momentRate.Mxz[pos] *= ( float )medium.mu[idx] * V;
		momentRate.Myz[pos] *= ( float )medium.mu[idx] * V;
	END_CALCULATE2D( )
