...
void modelChecking( PARAMS params  )
{
	...
	if ( params.useTerrain && params.gauss_hill )
	{
		printf( 
		"!!!!!!!!!!!!!!!!!!!!!!!WORNING!!!!!!!!!!!!!!!!!!!!!!!"
		"You set \"useTerrain\" and \"gauss_hill\" can not be 1 at the same time. You should configue your \"./paramsDir/paramsCGFDM3D-LSRK-CJMVS.json\" file. Or, we will use \"gauss_hill\" model to run the simulation\n"
		"!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
		params.gauss_hill = 1;
		params.useTerrain = 0;
...
void run( MPI_Comm comm_cart, MPI_COORD thisMPICoord, MPI_NEIGHBOR mpiNeighbor, GRID grid, PARAMS params )
{
...
#ifdef FLOAT16
	allocMediumFLOAT( grid, &medium_fp );
#endif
	vs_vp_rho2lam_mu_bou( medium_fp, medium, grid );


	//solve con matrix and release coordinate memory
	CONTRAVARIANT con;
	float * Jac;
	...
	solveContravariantJac( comm_cart, grid, con, coord, Jac, medium, _rDZ_DX, _rDZ_DY );
#else 
	solveContravariantJac( comm_cart, grid, con, coord, Jac );
#endif
	freeCoord( coord );
#ifdef GPU_CUDA
	...
	freeMedium( medium );
#else
	//medium_fp = (MEDIUM_FLOAT)medium;
	memcpy( &medium_fp, &medium, sizeof( MEDIUM_FLOAT ) );
#endif

	MPI_SendRecv_con_jac( comm_cart, grid, mpiNeighbor, con, Jac);
	MPI_Barrier( MPI_COMM_WORLD );

	CONTRAVARIANT_FLOAT con_fp;
	FLOAT * Jac_fp;
#ifdef FLOAT16
	allocContravariant_FLOAT( grid, &con_fp );
	allocJac_FLOAT( grid, &Jac_fp );
	matrixfloat2FLOAT( grid, con_fp, con, Jac_fp, Jac );
	freeContravariant( con );
	freeJac( Jac );
#else
	//con_fp = (CONTRAVARIANT_FLOAT)con;
...
#endif
	freeContravariant_FLOAT( con_fp );
	freeJac_FLOAT( Jac_fp );
	freeMediumFLOAT( medium_fp );
