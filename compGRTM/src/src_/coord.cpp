			...
			float DH, float cal_depth )
{
	...
	float a = 0.3 * cal_depth, b = 0.3 * cal_depth;
...
#ifdef GPU_CUDA
		CHECK( cudaMemcpy( cpu_coord.x, coord.x, size, cudaMemcpyDeviceToHost ));
#endif

	if ( useTerrain )
	{
#ifdef GPU_CUDA
		preprocessTerrain( params, comm_cart, thisMPICoord, grid, cpu_coord );
		CHECK( cudaMemcpy( coord.x, cpu_coord.x, size, cudaMemcpyHostToDevice ));
#else
		preprocessTerrain( params, comm_cart, thisMPICoord, grid, coord );
#endif
