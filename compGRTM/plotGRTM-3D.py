...
Vx = np.zeros( [stationNum, NT ] )
Vy = np.zeros( [stationNum, NT ] )
Vz = np.zeros( [stationNum, NT ] )

for it in range( 1, NT - 1 ):
	Vx[:, it] = ( Ux[:, it + 1] - Ux[:, it - 1] ) / DT * 0.5
	Vy[:, it] = ( Uy[:, it + 1] - Uy[:, it - 1] ) / DT * 0.5
	Vz[:, it] = ( Uz[:, it + 1] - Uz[:, it - 1] ) / DT * 0.5
...
np.save( "CompareData/GRTM3D/GRTMVx.npy", Vx )
np.save( "CompareData/GRTM3D/GRTMVy.npy", Vy )
np.save( "CompareData/GRTM3D/GRTMVz.npy", Vz )
